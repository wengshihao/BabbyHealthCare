...
public class CircleViewModel extends AndroidViewModel {
    ...
    private LiveData<List<CircleEntity>> allCircleEntityLive;
    public CircleViewModel(@NonNull Application application) {
        super(application);
        CircleDatabase circleDatabase =CircleDatabase.getDatabase(application);
        circleDao = circleDatabase.getCircleDao();
        allCircleEntityLive = circleDao.getAllCircleEntity();
        if(circleDao.getCountCircleEntity()==0){
            String[] text = {
            ...
            for(int i=0; i<text.length;i++){
                insertCircleEntity(new CircleEntity(text[i],0));
            }
        }
    }

    public LiveData<List<CircleEntity>> getAllCircleEntityLive() {
        return allCircleEntityLive;
    }

    void insertCircleEntity(CircleEntity... circleEntities){
        new InsertAsyncTask(circleDao).execute(circleEntities);
    }

    void clear(){
        new ClearAsyncTask(circleDao).execute();
    }


    static class InsertAsyncTask extends AsyncTask<CircleEntity,Void,Void> {
        private CircleDao circleDao;
        public InsertAsyncTask(CircleDao circleDao){
            this.circleDao = circleDao;
        }
        @Override
        protected Void doInBackground(CircleEntity... circleEntities) {
            circleDao.insert(circleEntities);
            return null;
        }
    }

    static class ClearAsyncTask extends AsyncTask<CircleEntity,Void,Void> {
        private CircleDao circleDao;
        public ClearAsyncTask(CircleDao circleDao){
            this.circleDao = circleDao;
        }
        @Override
        protected Void doInBackground(CircleEntity... circleEntities) {
            circleDao.deleteAllCircleEntity();
            return null;
        }
    }
